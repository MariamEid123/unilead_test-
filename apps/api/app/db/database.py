...
- Default (local dev): SQLite — ``sqlite:///./arete.db`` (zero-config).
...
_LIGHTWEIGHT_COLUMNS = {
    ...
    # Curriculum catalog: course cards carry a description (0007).
    "courses": [
        ("description", "TEXT NOT NULL DEFAULT ''", ""),
    ],
    # 4E: remediation plans now link to the triggering evidence + lifecycle.
    "remediation_plans": [
        ("evidence_id", "INTEGER", None),
        ("reason_codes_json", "TEXT NOT NULL DEFAULT '[]'", "[]"),
        ("status", "VARCHAR(16) NOT NULL DEFAULT 'open'", "open"),

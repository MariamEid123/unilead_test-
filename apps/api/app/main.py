"""Unified FastAPI entrypoint for the Arete platform.
...
settings = Settings()
_log = logging.getLogger("arete.main")
manager, orchestrator, reasoning_engine, provider = build_singletons(settings)

app = FastAPI(
    title="Arete API",
    description=(
        "Unified gateway for the Arete platform. Serves both the Compass "
...
app.include_router(student_model.router)
app.include_router(admin.router)
# Curriculum catalog (course/module/lesson views) — authenticated.
app.include_router(curriculum.router)
# Auth (signup/login/me) — wired to the DB.
app.include_router(auth.router)
# Code lab (Structured Programming) — real sandboxed execution + grading.

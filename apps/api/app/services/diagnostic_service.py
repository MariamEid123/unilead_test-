...
def submit_diagnostic(
    ...
    # the Areta-style results derived above.
    if http_request is not None:
        try:
            _run_ai_education_diagnostic(answers, http_request, student_id)
            # Sync the (possibly updated) competency state back to Areta.
            ai_education_bridge.sync_Areta_state_from_manager(
...
def _run_ai_education_diagnostic(
    ...
    for q in DIAGNOSTIC_QUESTIONS:
        ...
        mec271_id = ai_education_bridge.Areta_id_to_mec271(q["competency_id"])

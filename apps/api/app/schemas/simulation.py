...
class SimulationResult(BaseModel):
    ...
    # Sprint 7 evidence linkage — the run now routes through the standard
    # attempt -> rubric -> evidence -> mastery -> plan pipeline.
    attempt_id: int | None = Field(
        default=None,
        description="AssessmentAttempt id created for this run (audit lineage).",
    )
    evidence_id: int | None = Field(
        default=None,
        description="EvidenceRecord id written for this run (audit lineage).",
    )
    remediation_plan_id: int | None = Field(
        default=None,
        description="Remediation plan id opened on FAIL (None on PASS).",
    )
    mastery_level: str | None = Field(
        default=None,
        description="Resolved mastery level after this evidence (e.g. DEMONSTRATED).",
    )
